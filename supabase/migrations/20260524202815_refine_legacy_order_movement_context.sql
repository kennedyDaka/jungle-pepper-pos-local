create index if not exists stock_movements_sale_ref_item_idx
  on public.stock_movements (ref_type, type, ref_id, item_id)
  where type = 'sale';
...
left join lateral (
  ...
  select
    ...
    string_agg(
      distinct menu_item_name || ' x' || trim(to_char(order_item_qty, 'FM999999990.###')),
      ', '
      order by menu_item_name || ' x' || trim(to_char(order_item_qty, 'FM999999990.###'))
    ) as menu_item_names,
