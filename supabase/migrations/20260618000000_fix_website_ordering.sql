-- Fix create_website_order: security definer to bypass RLS for anon
-- New get_pending_orders RPC: security definer so cashiers see ALL pending orders
...
-- New RPC: get_pending_orders
-- Returns pending/preparing/ready/served orders with items and relations for a branch
-- Uses security definer so cashiers see ALL orders (not just their own)
