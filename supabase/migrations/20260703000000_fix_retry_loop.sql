-- Fix retry loop in create_pos_order: when _physical_order_no is provided
-- and the orders INSERT hits a unique_violation, the exception handler must
-- generate a new random receipt_ref instead of re-deriving it from the
-- same _physical_order_no (which would cause all 5 retries to fail with
-- the same collision, leaving order_id NULL).
