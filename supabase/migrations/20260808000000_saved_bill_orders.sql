-- POS "Save bill" to pending:
-- clones create_waiter_order semantics (payload unit_price overrides,
-- omissions with positive qty, per-line and standalone packaging,
-- no stock movement at creation) but records source = 'pos' so the
-- cashier can reopen the bill later, edit it and pay it on the POS.

create or replace function public.create_saved_bill(
  _payload jsonb,
  _branch_id uuid
)
returns uuid
...
declare
  ...
  note text;
  sale_type text := coalesce(nullif(btrim(coalesce(_payload ->> 'sale_type', '')), ''), 'regular');
...
begin
  ...
  values (
    ...
    sale_type,
    'pending',
    nullif(btrim(coalesce(_payload ->> 'note', '')), ''),
    'pos',
    now(),
    now()
  )
...
grant execute on function public.create_saved_bill(jsonb, uuid) to authenticated;