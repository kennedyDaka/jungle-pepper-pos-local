-- Fix ambiguity: use constraint name instead of column names in ON CONFLICT
...
begin
  ...
  for i in 1..5 loop
    begin
      insert into public.orders (
        branch_id, cashier_id, subtotal, discount, total,
        vat_rate, net_amount, vat_amount, sale_type, staff_meal_reason,
        status, note, source, physical_order_no, created_at, updated_at
      ) values (
        _branch_id, user_id, coalesce(subtotal, 0), coalesce(discount, 0), coalesce(total, 0),
        coalesce(vat_rate, 0.175), coalesce(net_amount, 0), coalesce(vat_amount, 0),
        resolved_sale_type, _staff_meal_reason, 'paid', note, 'pos',
        receipt_ref, resolved_sale_at, now()
      )
  ...
  for line in select * from jsonb_array_elements(item_lines) loop
    ...
    returning id into func.order_item_id;
