-- Fix discount null error when processing payment on pending orders
-- The process_payment RPC computes resolved_discount using:
--   coalesce(_discount, current_order.discount)
-- If _discount is null (not passed from UI) AND the order's discount is null
-- (e.g. orders created before the COALESCE fix in create_waiter_order), the
-- result is null, violating the NOT NULL constraint on orders.discount.

-- Fix existing null discounts before altering the function
