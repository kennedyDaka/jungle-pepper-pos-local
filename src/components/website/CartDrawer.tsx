...
export function CartButton() {
  ...
  return (
    ...
    >
      ...
      {count > 0 && (
        <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-[color:var(--brand-yellow)] px-1.5 text-[11px] font-bold text-[color:var(--brand-ink)]">{count}</span>
      )}
    </button>
  );
}
    ...
    <div className="fixed inset-0 z-50">
      ...
      <aside className="absolute inset-x-0 bottom-0 flex max-h-[92vh] flex-col rounded-t-3xl bg-[color:var(--brand-paper)] shadow-2xl sm:inset-y-0 sm:right-0 sm:left-auto sm:max-h-none sm:w-full sm:max-w-md sm:rounded-l-3xl sm:rounded-tr-none">
        ...
        <header className="flex items-center justify-between px-5 pt-3 pb-3 sm:pt-5">
          ...
          <button onClick={close} aria-label="Close" className="grid h-9 w-9 place-items-center rounded-full hover:bg-black/5"><X size={18} /></button>
        </header>
        ...
        {success ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center">
            ...
            <div className="text-sm space-y-1">
              <p>{success.customerName} · {success.phone}</p>
            ...
            <div className="w-full max-w-xs border-t border-[color:var(--brand-ink)]/10 pt-3 mt-1">
              {success.items.map((it, i) => (
                <p key={i} className="text-xs text-left">
                  ...
                  {it.modifiers && <span className="text-[color:var(--brand-muted)]"> ({it.modifiers.map((m) => m.name).join(", ")})</span>}
                </p>
              ))}
            </div>
            <p className="text-xs text-[color:var(--brand-muted)]">We'll call you shortly to confirm.</p>
            <button onClick={() => { setSuccess(null); close(); }} className="mt-4 rounded-full bg-[color:var(--brand-red)] px-6 py-3 font-semibold text-white">Done</button>
          </div>
        ) : items.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center">
            <p className="font-display text-xl">No items yet</p>
            <p className="text-sm text-[color:var(--brand-muted)]">Tap any dish on the menu to add it.</p>
          </div>
        ) : (
          <div className="flex flex-1 flex-col overflow-y-auto">
            <ul className="divide-y divide-[color:var(--border)] px-5">
              {items.map((it) => (
                <li key={it.id + it.modifiers.map((m) => m.modifier_id).join(",")} className="flex items-start gap-3 py-3">
                  <div className="min-w-0 flex-1">
                    <p className="font-display text-base uppercase leading-tight">{it.name}</p>
                    {it.modifiers.length > 0 && (
                      <p className="text-[10px] text-[color:var(--brand-muted)]">{it.modifiers.map((m) => m.name).join(", ")}</p>
                    )}
                    <p className="text-xs text-[color:var(--brand-muted)]">{formatMK(it.price_mwk)} each</p>
                    <div className="mt-2 inline-flex items-center rounded-full border border-[color:var(--border)] bg-white">
                      <button onClick={() => setQty(it.id, it.quantity - 1)} className="px-3 py-1 text-lg active:bg-black/5" aria-label="Decrease">−</button>
                      <span className="min-w-7 text-center text-sm font-semibold">{it.quantity}</span>
                      <button onClick={() => setQty(it.id, it.quantity + 1)} className="px-3 py-1 text-lg active:bg-black/5" aria-label="Increase">+</button>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-display text-[color:var(--brand-red)]">
                      {formatMK((it.price_mwk + it.modifiers.reduce((s, m) => s + m.price_delta, 0)) * it.quantity)}
                    </p>
                    <button onClick={() => remove(it.id)} className="mt-1 text-[10px] uppercase tracking-wider text-[color:var(--brand-muted)]">Remove</button>
                  </div>
                </li>
              ))}
            </ul>

            <form onSubmit={onSubmit} className="mt-2 space-y-3 border-t border-[color:var(--border)] bg-[color:var(--brand-paper)] px-5 py-4">
              <div className="grid grid-cols-2 gap-2">
                {(["pickup", "dine_in"] as const).map((t) => (
                  <label key={t} className={`cursor-pointer rounded-xl border-2 px-3 py-2.5 text-center text-xs font-bold uppercase tracking-wider transition ${orderType === t ? "border-[color:var(--brand-red)] bg-[color:var(--brand-red)] text-white" : "border-[color:var(--border)] bg-white"}`}>
                    <input type="radio" name="order_type" value={t} checked={orderType === t} onChange={() => setOrderType(t)} className="sr-only" />
              ...
              <Field name="pickup_time" type="time" required defaultValue="18:30" label={orderType === "pickup" ? "Pickup time" : "Arrival time"} />
              <textarea name="notes" rows={2} maxLength={500} placeholder="Notes (allergies, table preference…)" className="w-full rounded-xl border border-[color:var(--border)] bg-white px-3 py-2.5 text-sm" />
              {error && <p className="rounded-lg bg-[color:var(--brand-red)]/10 px-3 py-2 text-xs text-[color:var(--brand-red)]">{error}</p>}
              <div className="flex items-center justify-between pt-1">
                <span className="text-sm text-[color:var(--brand-muted)]">Total ({count})</span>
                <span className="font-display text-2xl text-[color:var(--brand-red)]">{formatMK(subtotal)}</span>
              </div>
              <button disabled={submitting} className="w-full rounded-full bg-[color:var(--brand-red)] py-3.5 text-sm font-bold uppercase tracking-wider text-white shadow-lg shadow-[color:var(--brand-red)]/30 active:scale-[0.99] disabled:opacity-50">
                {submitting ? "Sending order…" : "Send Order"}
              </button>
              <p className="pb-2 text-center text-[10px] text-[color:var(--brand-muted)]">By placing the order you agree we may contact you to confirm.</p>
            </form>
          </div>
        )}
      </aside>
  ...
  if (label) {
    return (
      <label className="block">
        <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wider text-[color:var(--brand-muted)]">{label}</span>
        <input {...props} className="w-full rounded-xl border border-[color:var(--border)] bg-white px-3 py-2.5 text-sm" />
      </label>
    );
  }
  return <input {...props} className="w-full rounded-xl border border-[color:var(--border)] bg-white px-3 py-2.5 text-sm" />;
}

export function AddToCartButton({ item, className, onAdd }: {
  ...
  return (
    <button
      ...
      className={className ?? "rounded-full bg-[color:var(--brand-red)] px-4 py-2 text-xs font-semibold uppercase tracking-wider text-white"}
    >
      Add
    </button>
  );
