...
export const Route = createFileRoute("/website/menu")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Full menu: Portuguese starters, salads, wood-fired pizza, burgers, pasta, prego, frango, prawns, drinks & more." },
    ],
  }),
  ...
  component: MenuPage,
  errorComponent: ({ error }) => <div className="p-8 text-center">Couldn't load menu: {error.message}</div>,
});
...
function MenuPage() {
  ...
  const handleAdd = (item: any) => {
    ...
    add({ id: item.id, slug: item.slug, name: item.name, price_mwk: item.price, kind: item.kind }, 1);
  };

  return (
    <div>
      <section className="bg-[color:var(--brand-red)] py-8 text-center text-[color:var(--brand-paper)] sm:py-12">
        <p className="font-script text-xl text-[color:var(--brand-yellow)] sm:text-3xl">Welcome to</p>
        <h1 className="font-display text-4xl leading-none tracking-wide sm:text-6xl md:text-7xl">JUNGLE PEPPER</h1>
        <p className="mt-2 text-[10px] uppercase tracking-[0.25em] opacity-90 sm:text-sm sm:tracking-[0.3em]">Malawi's Own Pizza · Portuguese Cuisine</p>
      </section>
      ...
      <nav className="sticky top-[57px] z-30 border-y border-[color:var(--border)] bg-[color:var(--brand-paper)]/95 backdrop-blur sm:top-[68px]">
        <div className="mx-auto flex max-w-6xl gap-2 overflow-x-auto px-3 py-2.5 text-[11px] font-semibold uppercase tracking-wider scrollbar-thin sm:text-xs">
          {categories.map((c) => (
            <a key={c.slug} href={`#cat-${c.slug}`} className="whitespace-nowrap rounded-full border border-[color:var(--brand-red)]/30 px-3 py-1.5 active:bg-[color:var(--brand-red)] active:text-white hover:bg-[color:var(--brand-red)] hover:text-white">
              {c.name}
            </a>
          ))}
        </div>
          ...
          return (
            <section key={cat.slug} id={`cat-${cat.slug}`} className="scroll-mt-36 mb-10 sm:mb-14">
              <div className="mb-4 text-center sm:mb-6">
                <h2 className="font-display text-2xl text-[color:var(--brand-red)] sm:text-4xl md:text-5xl">{cat.name.toUpperCase()}</h2>
...
function MenuRow({ item, onAdd }: { item: any; onAdd: () => void }) {
  return (
    <li>
      <button type="button" onClick={onAdd} className="menu-line w-full text-left" aria-label={`Add ${item.name} to order`}>
        <span className="min-w-0 text-left">
          <span className="font-display text-base uppercase leading-tight tracking-wide sm:text-lg">{item.name}</span>
          {item.description && (
            <span className="mt-0.5 block text-xs text-muted-foreground sm:text-sm">{item.description}</span>
          )}
        </span>
        <span className="font-display text-base text-[color:var(--brand-red)] whitespace-nowrap sm:text-lg">{formatMK(item.price)}</span>
      </button>
    </li>
  );
}
