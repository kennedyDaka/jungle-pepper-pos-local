      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; }
          ...
          .ticket-row { display: flex; justify-content: space-between; gap: 8px; flex-wrap: wrap; }
          .ticket-row > span:first-child { min-width: 0; overflow-wrap: break-word; word-break: break-word; }
...
function PinGate({
  ...
  const handleVerify = async () => {
    ...
    setVerifying(true);
    try {
      const { data, error: rpcError } = await (supabase as any).rpc("verify_branch_pin", {
  ...
  if (error) {
    return (
      <div className="min-h-screen">
        <ErrorState error={error} label="Could not load branch" />
      </div>
    );
  }
  if (!branch) {
    return (
      <div className="min-h-screen">
        <ErrorState error="No active branch found" label="Contact admin" />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <Card className="max-w-sm w-full p-8 text-center space-y-6">
        ...
        <div className="space-y-3">
          ...
          <Button className="w-full" disabled={pin.length < 4 || verifying} onClick={handleVerify}>
...
function OrderPage({ branchId, branchName }: { branchId: string; branchName: string }) {
    ...
    setLines((prev) => {
      const existing = prev.find(
        (l) =>
          ...
          l.price === price &&
          l.modifiers.length === modifiers.length &&
          l.modifiers.every((m, i) => m.id === modifiers[i]?.id),
      );
      ...
      return [
        ...prev,
        { key: crypto.randomUUID(), menuItemId, name: displayName, price, qty: 1, modifiers },
      ];
    });
  };

  const incQty = (key: string) =>
    setLines((p) => p.map((l) => (l.key === key ? { ...l, qty: l.qty + 1 } : l)));
  const decQty = (key: string) =>
    setLines((p) =>
      p.flatMap((l) => (l.key === key ? (l.qty > 1 ? [{ ...l, qty: l.qty - 1 }] : []) : [l])),
    );
  ...
  const subtotal = useMemo(
    () =>
      lines.reduce(
        (s, l) => s + (l.price + l.modifiers.reduce((m, mod) => m + mod.price_delta, 0)) * l.qty,
        0,
      ),
    [lines],
  );
  ...
  const handlePizzaCrust = (crustId: string, crustName: string, priceDelta: number) => {
    ...
    addToCart(crustDialog.dbName, crustDialog.name + ` (${crustName})`, crustDialog.price, [
      { id: realId, name: crustName, price_delta: priceDelta },
    ]);
    setCrustDialog(null);
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto flex max-w-[1500px] gap-6 p-4 lg:p-6">
        <div className="flex-1 space-y-6">
          ...
          <button
            type="button"
            onClick={() =>
              setCrustDialog({ itemKey: "test", name: "", dbName: "", price: 0, menuItemId: "" })
            }
            style={{ display: "none" }}
          />
          <ExtrasFolder
          ...
          <div className="my-4 flex-1 overflow-y-auto pr-1 space-y-2">
            {lines.length === 0 ? (
              ...
              lines.map((l) => (
                <div
                  key={l.key}
                  className="flex items-center gap-2 rounded-lg border border-border/60 bg-secondary/30 p-2.5"
                >
          ...
          <div className="space-y-3 border-t border-border pt-4">
            ...
            <Button
              ...
              disabled={
                lines.length === 0 ||
                submit.isPending ||
                (service === "dine-in" && !selectedTableId)
              }
              className="w-full gap-2 bg-primary text-base font-semibold hover:bg-primary/90"
              size="lg"
            >
      ...
      {submitResult && (
        <OrderReceiptDialog result={submitResult} onClose={() => setSubmitResult(null)} />
      )}
    </div>
  );
}
            ...
            {lines.map((l) => (
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => onDec(l.key)} className="h-7 w-7 grid place-items-center">
                    <Minus className="h-3.5 w-3.5" />
                  </button>
                  <span className="w-5 text-center text-xs">{l.qty}</span>
                  <button onClick={() => onInc(l.key)} className="h-7 w-7 grid place-items-center">
                    <Plus className="h-3.5 w-3.5" />
                  </button>
                </div>
                <button onClick={() => onRemove(l.key)}>
                  <X className="h-4 w-4" />
                </button>
              </div>
            ))}
...
function DrinksFolder({
  ...
  livePrice: (dbName: string) => number | undefined;
}) {
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <button
          type="button"
          className="group flex w-full items-center gap-4 rounded-xl border border-border bg-card p-4 text-left shadow-sm transition hover:border-primary/60 hover:bg-primary/5 active:scale-[0.997]"
        >
          ...
          <span className="rounded-full border border-border bg-secondary/40 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground group-hover:text-foreground">
            Open
          </span>
        </button>
      </DialogTrigger>
      <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Wine className="h-5 w-5 text-primary" /> Drinks & Liquor
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-5 pt-2">
          {EXTRA_MENU.map((cat) => (
            <section key={cat.id}>
              <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-primary/90">
                {cat.label}
              </h3>
                ...
                {cat.items.map((it) => (
                  ...
                  >
                    ...
                    <span className="text-[11px] font-semibold tabular-nums text-primary">
                      {formatMK(livePrice(it.dbName) ?? it.price)}
                    </span>
                  </button>
                ))}
              </div>
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <ChefHat className="h-5 w-5 text-primary" /> Extras
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-5 pt-2">
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {catItems.map((it: any) => (
                    ...
                    >
                      ...
                      <span className="text-[11px] font-semibold tabular-nums text-primary">
                        {formatMK(livePrice(it.name) ?? Number(it.price))}
                      </span>
                    </button>
                  ))}
                </div>
      ...
      <div className="relative w-full" style={{ aspectRatio: `${PDF_W} / ${PDF_H}` }}>
        ...
        {items.map((it) => {
          ...
          return (
            <button
              key={it.id}
              onClick={() => onAdd({ name: it.name, dbName: it.dbName, price, kind: it.kind })}
...
function CrustDialog({
  open,
  onSelect,
  onClose,
}: {
  open: boolean;
  onSelect: (id: string, name: string, priceDelta: number) => void;
  onClose: () => void;
}) {
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-sm">
        ...
        <div className="grid grid-cols-2 gap-3 py-4">
          <button
            onClick={() => onSelect("thin-crust", "Thin Crust", 0)}
          ...
          <button
            onClick={() => onSelect("thick-crust", "Thick Crust", 0)}
            className="rounded-lg border border-border p-4 text-center font-medium hover:border-primary hover:bg-primary/10 transition"
          >
            Thick Crust
          </button>
...
function ShapeDialog({
  ...
  open: boolean;
  sauce: string;
  onSelect: (shape: string, dbName: string) => void;
  onClose: () => void;
}) {
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-sm">
        ...
        <div className="grid grid-cols-3 gap-3 py-4">
          {PASTA_OPTIONS.map((opt) => (
            <button
              key={opt.shape}
              onClick={() => onSelect(opt.shape, opt.dbName.replace("{name}", sauce))}
              className="rounded-lg border border-border p-4 text-center font-medium hover:border-primary hover:bg-primary/10 transition"
            >
              {opt.shape}
            </button>
            ...
            >
              ...
              {result.lines.map((l: any) => (
                <div key={l.key} className="ticket-row mb-1">
                  ...
                  <span>
                    {MWK(
                      (l.price + l.modifiers.reduce((s: number, m: any) => s + m.price_delta, 0)) *
                        l.qty,
                    )}
                  </span>
                </div>
